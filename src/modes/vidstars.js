...
function init() {
  paused = false;
  if (!ready && !el.getAttribute('src')) { el.src = '/textflow/static/startrails.mp4'; el.load(); }
  el.currentTime = 0; el.play().catch(function(){});
}
function render() {
  clearCanvas();
  var W = state.COLS, H = state.ROWS;
  if (!ready) { drawFancyLoading('loading star trails'); return; }
  ...
  for (var y = 0; y < H; y++) {
    for (var x = 0; x < W; x++) {
      ...
      // Night sky — boost whites, add subtle blue
      var br = Math.min(255, r * 1.1);
      var bg = Math.min(255, g * 1.05 + 5);
      var bb = Math.min(255, b * 1.2 + 15);
      drawChar(VA_RAMP[ri], x, y, br|0, bg|0, bb|0, Math.max(0.15, Math.min(1, lum*1.8)));
    }
  }
...
function attach() {
  state.canvas.addEventListener('click', function() {
    if (state.currentMode !== 'vidstars') return;
    if (el.paused) { el.play().catch(function(){}); paused=false; } else { el.pause(); paused=true; }
  });
}
registerMode('vidstars', { init: init, render: render, attach: attach });
