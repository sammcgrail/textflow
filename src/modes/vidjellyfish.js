...
function init() {
  paused = false;
  if (!ready && !el.getAttribute('src')) { el.src = '/textflow/static/jellyfish.mp4'; el.load(); }
  el.currentTime = 0; el.play().catch(function(){});
}
function render() {
  clearCanvas();
  var W = state.COLS, H = state.ROWS;
  if (!ready) { drawFancyLoading('loading jellyfish'); return; }
  ...
  for (var y = 0; y < H; y++) {
    for (var x = 0; x < W; x++) {
      ...
      // Boost deep blues and cyans for jellyfish glow
      var maxC = Math.max(r, g, b);
      var boost = maxC > 0 ? 255 / maxC : 1;
      var br = Math.min(255, r * boost * 0.6 + r * 0.4);
      var bg = Math.min(255, g * boost * 0.6 + g * 0.4);
      var bb = Math.min(255, b * boost * 0.6 + b * 0.4);
...
function attach() {
  state.canvas.addEventListener('click', function() {
    if (state.currentMode !== 'vidjellyfish') return;
    if (el.paused) { el.play().catch(function(){}); paused=false; } else { el.pause(); paused=true; }
  });
}
registerMode('vidjellyfish', { init: init, render: render, attach: attach });
