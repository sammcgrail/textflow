...
var jCr, jCi, jDirty, jImage, jW, jH, jAutoOrbit;
...
function renderJulia() {
  ...
  if (pointer.clicked && state.currentMode === 'julia') {
    ...
    var charAspect = (state.CHAR_W || 6) / (state.CHAR_H || 12.5);
    var screenAspect = W / H * charAspect;
    jCr = (pointer.gx / W - 0.5) * 3.0 * screenAspect;
    jCi = (pointer.gy / H - 0.5) * 3.0;
    jAutoOrbit = false;
    jDirty = true;
  }

  // Auto orbit c parameter
  if (jAutoOrbit) {
    var orbitSpeed = 0.15;
    jCr = 0.7885 * Math.cos(t * orbitSpeed);
    jCi = 0.7885 * Math.sin(t * orbitSpeed);
    jDirty = true;
  }
