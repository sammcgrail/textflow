...
function init() {
  paused = false;
  if (!ready && !el.getAttribute('src')) { el.src = '/textflow/static/gears.mp4'; el.load(); }
  el.currentTime = 0; el.play().catch(function(){});
}
function render() {
  clearCanvas();
  var W = state.COLS, H = state.ROWS;
  if (!ready) { drawFancyLoading('loading gears'); return; }
  ...
  for (var y = 0; y < H; y++) {
    for (var x = 0; x < W; x++) {
      ...
      // Warm metallic tint
      var br = Math.min(255, r * 1.2 + 30);
      var bg = Math.min(255, g * 0.9 + 15);
      var bb = Math.min(255, b * 0.6);
...
function attach() {
  state.canvas.addEventListener('click', function() {
    if (state.currentMode !== 'vidgears') return;
    if (el.paused) { el.play().catch(function(){}); paused=false; } else { el.pause(); paused=true; }
  });
}
registerMode('vidgears', { init: init, render: render, attach: attach });
