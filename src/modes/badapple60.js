...
// ============================================================
// BadApple60 — same shadow-animation as `badapple`, but the source
// video is motion-interpolated to a true 60fps (vs the 30fps-native
// frame-doubled `badapple`). Smoother in-between motion at the cost of
// occasional silhouette morph artifacts.
...
function initBadapple60() {
  videoPaused = false;
  apples = [];
  if (!videoReady && !videoEl.getAttribute('src')) {
    videoEl.src = '/textflow/static/bad-apple60.mp4';
    videoEl.load();
  }
...
function renderBadapple60() {
  ...
  if (!videoReady) {
    drawFancyLoading('loading bad apple 60');
    return;
  }
  ...
  if (pointer.clicked && state.currentMode === 'badapple60') {
...
function attachBadapple60() {
  var lastClick = 0;
  state.canvas.addEventListener('click', function() {
    if (state.currentMode !== 'badapple60') return;
    var now = performance.now();
...
registerMode('badapple60', {
  init: initBadapple60,
  render: renderBadapple60,
  attach: attachBadapple60,
});
