...
function init() {
  paused = false;
  if (!ready && !el.getAttribute('src')) { el.src = '/textflow/static/oceanwaves.mp4'; el.load(); }
  el.currentTime = 0; el.play().catch(function(){});
}
function render() {
  clearCanvas();
  var W = state.COLS, H = state.ROWS;
  if (!ready) { drawFancyLoading('loading ocean'); return; }
  ...
  for (var y = 0; y < H; y++) {
    for (var x = 0; x < W; x++) {
      ...
      // Cool ocean boost — enhance blues
      var br = Math.min(255, r * 0.6);
      var bg = Math.min(255, g * 0.9);
      var bb = Math.min(255, b * 1.3);
...
function attach() {
  state.canvas.addEventListener('click', function() {
    if (state.currentMode !== 'vidocean') return;
    if (el.paused) { el.play().catch(function(){}); paused=false; } else { el.pause(); paused=true; }
  });
}
registerMode('vidocean', { init: init, render: render, attach: attach });
