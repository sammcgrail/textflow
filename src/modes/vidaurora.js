...
function init() {
  paused = false;
  if (!ready && !el.getAttribute('src')) { el.src = '/textflow/static/aurora-vid.mp4'; el.load(); }
  el.currentTime = 0; el.play().catch(function(){});
}
function render() {
  clearCanvas();
  var W = state.COLS, H = state.ROWS;
  if (!ready) { drawFancyLoading('loading aurora'); return; }
  ...
  for (var y = 0; y < H; y++) {
    for (var x = 0; x < W; x++) {
      ...
      // Green/purple aurora tint
      var br = Math.min(255, r * 0.7 + b * 0.3);
      var bg = Math.min(255, g * 1.3);
      var bb = Math.min(255, b * 1.1 + r * 0.1);
      drawChar(VA_RAMP[ri], x, y, br|0, bg|0, bb|0, Math.max(0.15, Math.min(1, lum*1.6)));
    }
  }
...
function attach() {
  state.canvas.addEventListener('click', function() {
    if (state.currentMode !== 'vidaurora') return;
    if (el.paused) { el.play().catch(function(){}); paused=false; } else { el.pause(); paused=true; }
  });
}
registerMode('vidaurora', { init: init, render: render, attach: attach });
