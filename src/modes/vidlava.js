...
function init() {
  paused = false;
  if (!ready && !el.getAttribute('src')) { el.src = '/textflow/static/lava-flow.mp4'; el.load(); }
  el.currentTime = 0; el.play().catch(function(){});
}
function render() {
  clearCanvas();
  var W = state.COLS, H = state.ROWS;
  if (!ready) { drawFancyLoading('loading lava'); return; }
  ...
  for (var y = 0; y < H; y++) {
    for (var x = 0; x < W; x++) {
      ...
      // Warm boost for lava oranges/reds
      var br = Math.min(255, r * 1.2);
      var bg = Math.min(255, g * 0.8);
      var bb = Math.min(255, b * 0.4);
      drawChar(VA_RAMP[ri], x, y, br|0, bg|0, bb|0, Math.max(0.25, Math.min(1, lum*1.4)));
    }
  }
...
function attach() {
  state.canvas.addEventListener('click', function() {
    if (state.currentMode !== 'vidlava') return;
    if (el.paused) { el.play().catch(function(){}); paused=false; } else { el.pause(); paused=true; }
  });
}
registerMode('vidlava', { init: init, render: render, attach: attach });
