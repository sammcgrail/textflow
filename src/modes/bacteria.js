...
function initBacteria() {
  ...
  bacStep = 0;
  // Seed more colonies spread across screen
  for (var c = 0; c < 12; c++) {
    var cx = (Math.random() * bacW) | 0, cy = (Math.random() * bacH) | 0;
    var type = 1 + (c % 4);
    // Seed a small cluster, not just one cell
    for (var dy = -1; dy <= 1; dy++) for (var dx = -1; dx <= 1; dx++) {
      var px = cx + dx, py = cy + dy;
      if (px >= 0 && px < bacW && py >= 0 && py < bacH) bacGrid[py * bacW + px] = type;
    }
  }
}
function renderBacteria() {
  ...
  if (pointer.clicked && state.currentMode === 'bacteria') {
    ...
    var type = 1 + ((Math.random() * 4) | 0);
    for (var dy = -2; dy <= 2; dy++) for (var dx = -2; dx <= 2; dx++) {
      var px = gx + dx, py = gy + dy;
      if (px >= 0 && px < W && py >= 0 && py < H) bacGrid[py * W + px] = type;
    }
  }
  var curStep = (state.time * 20) | 0;
  while (bacStep < curStep) {
    bacStep++;
    // More growth per step — try more cells
    for (var i = 0; i < 80; i++) {
      ...
      if (nx >= 0 && nx < W && ny >= 0 && ny < H && bacGrid[ny * W + nx] === 0) {
        if (Math.random() < 0.7) bacGrid[ny * W + nx] = type;
      }
    }
  }
  var chars = '.oO@#';
  ...
  for (var y = 0; y < H; y++) {
    for (var x = 0; x < W; x++) {
      ...
      var ci = Math.min(chars.length - 1, (n / 2) | 0);
      drawCharHSL(chars[ci], x, y, hues[v % hues.length], 70, (15 + n * 4) | 0);
    }
  }
}
